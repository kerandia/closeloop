...
- **Who makes the next touch** — an executor control, channel-agnostic: *the AI agent executes the touch* vs *the rep does it by hand*, whatever the `channel` (email / sms / whatsapp / phone / visit). Show the system's **suggested** choice + its one-line **reason**, with the rep able to confirm or override (sets `chosen`). `visit` is always rep-executed (the AI can't show up at a door) — render it rep-only. This is the control that keeps the installer in charge of the AI — they decide, per action, when it acts for them.
...
- **[Approve & Compose]** primary button → calls `POST /api/recommendations/:id/approve` (runs COMPOSE, returns the full `{message}` draft) → opens the Compose drawer with that draft
- secondary: **Dismiss**

### B. Compose drawer (opens on Approve)
- Channel-appropriate editor: subject (email only) + body, prefilled from COMPOSE
- Editable (`PATCH /api/messages/:id` on blur/save)
- **L1 (decided):** the drawer shows the **complete** draft returned by `approve` immediately — no streaming, so the Approve→Compose path is never blocked on a backend change.
- **L2 enhancement:** once COMPOSE is exposed as a streaming endpoint, the body **streams in token by token** as it generates instead of landing whole (see flag below).
- **[Send]** → `POST /api/messages/:id/send` → toast "Sent ✓", drawer closes, a new interaction appears in the timeline (L1: mocked send; the *visible* draft is what matters)
  > ⚠️ **Backend dependency / flag (L2 only):** token-by-token rendering needs COMPOSE and RESPOND exposed as **streaming endpoints** (SSE / per-line events). This is an enhancement — L1 works on the existing non-streaming `{message}` (COMPOSE) and JSON (RESPOND) responses. ANALYZE stays a normal request/response either way. See PR flags.
...
- **Respond:** input "What did the customer just say?" → submit → card with **read · exact lines (copyable) · why** (`POST /api/copilot/respond`, structured JSON: `read`, `exact_lines[]`, `why`). This is a killer live moment. To keep the live feel, **progressively render the human-readable fields** — reveal `exact_lines[]` one line at a time (per-line SSE events in L2) — *never* render raw partial JSON.
  - **L1 stub (frontend decision):** the panel is fully interactive but driven by a canned/mock response (the Müller-path answer) — input + submit + the progressive line reveal all run against the mock, so it reads as a live feature, never a broken one. L2 swaps the mock for the live `POST /api/copilot/respond` stream; the UI is unchanged.
- **Collect:** a "Suggest next question" button → shows the gap-filling question (`GET /api/copilot/collect/:id`).

### F. Interaction timeline
Reverse-chronological `interactions`: channel icon, time, summary/outcome, expandable transcript (markdown) for calls/visits. A **"+ Log visit / note"** action opens a small form (note + **gut-feel** field + outcome) → `POST /api/customers/:id/interactions` → the profile + recommendation visibly **update** (great demo beat: log "wife hesitant" → watch the next play change to spouse-targeted).
